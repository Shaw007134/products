...
INNER JOIN dt_users du ON (v.uid = du.id)
  ...
  static async visitors(ctx) {
    try {
      ...
      // 1.0 获取当前登录用户信息
      let currentUser = await ctx.executeSql(
        `select  * from dt_users where mobile='${ctx.state.user.name}'`
      )
  ...
  static async todayBest(ctx) {
    ...
    try {
      ...
      if (currentUser && currentUser.length > 0 && currentUser[0].age > 0) {
        innerwhere += ` and gender = '${
          currentUser[0].gender == '女' ? '男' : '女'
        }'`
  ...
  static async personalInfo(ctx) {
    try {
      ...
      // 1.0 获取当前登录用户和目标用户信息
      let users = await ctx.executeSql(
        `select * from dt_users where id = ${currentUserID} or id = ${uid}`
      )
      ...
      return ctx.send(targent_User, undefined, {
        counts,
        pagesize,
        pages,
        page
  ...
  static async personalInfoByGuid(ctx) {
    try {
      ...
      // 1.0 获取当前登录用户和目标用户信息
      let users = await ctx.executeSql(
        `select * from dt_users where  guid in (${guids})`
      )
  ...
  static async cards(ctx) {
    try {
      ...
      // 1.0 获取当前登录用户和目标用户信息
      let currentUser = await ctx.executeSql(
        `select * from dt_users where id = ${currentUserID}`
      )
  ...
  static async questions(ctx) {
    try {
      ...
      let tested = await ctx.executeSql(
        `select * from dt_question_user_ans where uid = ${currentUserID}`
      )
      ...
      let newdata = data.map((item, index) => {
        ...
        if (index == 0) {
          ...
          if (
            islockdata[index - 1] == false &&
            istesteddata[index - 1] == true
          ) {
            islockdata[index] = false
          } else {
  ...
  static async questionSection(ctx) {
    try {
      ...
      let data = await ctx.executeSql(
        `SELECT qsid, question_title FROM dt_question_section WHERE qid = ${qid} order by sortid asc`
      )
      if (!data || data.length <= 0) {
        return ctx.sendError(config.resCodes.customerError, '无问卷数据')
      }

      let ansdata = await ctx.executeSql(
        `select qsid,ans_title,ans_No from dt_questions_section_ans where qid = ${qid}`
      )
  ...
  static async questionsAns(ctx) {
    try {
      ...
      let question = await ctx.executeSql(
        `SELECT * FROM dt_questions where qid = ${qid}`
      )

      // 获取当前问卷的所有鉴定单数据
      let data = await ctx.executeSql(
        `SELECT * FROM dt_question_result where qid = ${qid}`
      )
      let datalng = 0
      ...
      let extis_result = await ctx.executeSql(
        `SELECT * FROM dt_question_user_ans where qid = ${qid} and uid=${currentUserID}`
      )
