...
class UserController {
  ...
  static async login(ctx) {
    ...
    try {
      console.log('get code for phone:  ', data)
      let vcode = 888888 //createRandom(6);
      let resultData = await ctx.executeSql(
        `select status from dt_users where mobile =${data.phone} `
      )
      if (resultData && resultData.length > 0) {
        // 更新验证码
        await ctx.executeSql(
          ` update dt_users set vcode = ${vcode},login_time=NOW() where mobile =${data.phone} `
        )
      } else {
        // db中没有该手机号，插入一条新的status为2的新记录
        await ctx.executeSql(
          ` insert into dt_users (mobile,vcode,status,login_time,guid) values (${
            data.phone
          },${vcode},2,NOW(),${data.phone + '' + Date.now()}) `
        )
      }
  ...
  static async loginVerification(ctx) {
    ...
    try {
      // 获取手机号码的验证码
      let resultData = await ctx.executeSql(
        `select id,guid,vcode,status from dt_users where mobile =${phone}`
      )
  ...
  static async uploadHead(ctx) {
    ...
    try {
      ...
      return ctx.send(
        {
          headImgShortPath:
            '/upload/' + `${Date.now() + '' + mobile + extname}`,
